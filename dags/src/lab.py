...
def calculate_cluster_stats(data_b64: str, n_clusters: int = 3):
    """
    Decodes preprocessed data, performs KMeans clustering,
    and prints stats to Airflow logs.

    Returns:
        dict: Cluster statistics (JSON-safe types only).
    """
    import numpy as np
    ...
    cluster_counts_raw = dict(Counter(labels))
    cluster_counts = {int(k): int(v) for k, v in cluster_counts_raw.items()}

    centers = kmeans.cluster_centers_.tolist()  # list of lists of floats
